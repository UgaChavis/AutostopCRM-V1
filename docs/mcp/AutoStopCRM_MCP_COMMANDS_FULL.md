...
| Команда | Тип | Что дает |
| --- | --- | --- |
| `list_clients` | read | Список клиентов с ФИО/названием, типом, телефонами и опциональной короткой статистикой. Для большого справочника лучше держать `limit=50..100` и `include_stats=false`, если нужен только обзор. |
| `search_clients` | read | Ищет клиента по ФИО, части ФИО, названию организации, телефону, email, ИНН, реквизитам, контактному лицу, автомобилю, госномеру или VIN из связанных карточек и сохраненного профиля клиента. |
...
| `create_client` | write | Создает запись клиента. Поддерживает физлицо, ИП, ООО, другую организацию, до 3 телефонов в `phones[]` и сохраненные `vehicles[]` клиента. `phone` остается первым/основным номером. |
| `update_client` | write | Обновляет профиль клиента, реквизиты, до 3 телефонов в `phones[]` и сохраненные `vehicles[]`. Передавать только изменяемые поля. |
| `delete_client` | destructive | Удаляет профиль клиента. По умолчанию отказывает, если есть связанные карточки; `allow_linked=true` использовать только после подтверждения. |
...
Для больших клиентских массивов:
...
- В профиле клиента можно хранить до 3 телефонов: `phone` остается первым/основным, `phones[]` содержит весь компактный список без дублей.
- Телефоны в формате `+7...` и `8...` считаются одним номером при поиске, подсказках и сборе истории клиента; поиск работает по любому номеру из `phones[]`.
- `overwrite_card_fields=true` в `link_card_to_client` использовать только после явного подтверждения пользователя.
- `delete_client` не удаляет карточки. Без `allow_linked=true` команда безопасно блокирует удаление связанного клиента; с `allow_linked=true` сначала снимает связи с карточек.
...
- `allow_linked`: для `delete_client` оставлять `false`, если пользователь явно не подтвердил удаление клиента со снятием связей с карточек.
