...
| `create_client` | write | Создает запись клиента. Поддерживает физлицо, ИП, ООО, другую организацию и сохраненные `vehicles[]` клиента. |
| `update_client` | write | Обновляет профиль клиента, реквизиты и сохраненные `vehicles[]`. Передавать только изменяемые поля. |
