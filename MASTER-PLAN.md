...
- локальный HTTP API
- MCP transport layer
- Telegram AI Board Manager worker
- legacy server AI worker / card indicator compatibility contour
...
Telegram AI Board Manager
    ->
OpenAI decision / voice / vision
    ->
CRM tool registry
    ->
BoardApiClient / Local HTTP API
    ->
Patch / verify / audit / Telegram report
```
...
```text
0. AutoStop CRM Product Envelope v1.7
  ...
  6. Server AI Contour v1.7
    6.1 Telegram AI Board Manager v1.0
    6.2 CRM tool registry v1.0
    6.3 Audit and verification v1.0
    6.4 Voice/photo intake v1.0
    6.5 Legacy agent compatibility v1.3
...
| `6` | Server AI Contour | `v1.7` | active | `main_telegram_ai.py`, `src/minimal_kanban/telegram_ai/`, `src/minimal_kanban/agent/control.py` |
| `7` | Browser UI Surface | `v1.5` | active | `src/minimal_kanban/web_assets.py` |
| `8` | Tests and Diagnostics | `v1.4` | stable | `tests/test_api.py`, `tests/test_telegram_ai.py`, `tests/test_mcp.py`, `scripts/check_live_connector.py` |
...
- desktop bootstrap
- MCP-only runtime
- поддержка Telegram AI worker через `main_telegram_ai.py`
- legacy agent runtime через `src/minimal_kanban/agent/*`; отдельного entrypoint `main_agent.py` в этой ветке нет
...
### 6.6 Module 6: Server AI Contour v1.7
...
- `Telegram -> authorize -> normalize -> context -> OpenAI decision -> CRM tools -> write -> verify -> audit -> reply`
...
- отдельный Telegram long-polling worker без публичного порта
- owner authorization по Telegram user ID
- text/voice/photo intake
- OpenAI Responses API decision JSON
- OpenAI audio transcription
- OpenAI vision extraction for photo facts
- explicit CRM tool registry
- CRM writes only through `BoardApiClient` / local HTTP API
- read-after-write verification
- redacted JSONL audit per run

Что ещё остаётся главным ограничением:

- rollback реализован для базовых обратимых действий, но пока не является универсальным transaction layer
- autopilot skeleton выключен по умолчанию
- documents/PDF from Telegram are normalized but not fully processed yet
...
Telegram update
  -> owner authorization
  -> text / voice / photo normalization
  -> CRM context builder
  -> OpenAI structured decision
  -> explicit CRM tool registry
  -> write through local API
  -> read-after-write verification
  -> redacted audit
  -> Telegram reply
```
...
- Telegram AI worker вынесен в отдельный entrypoint и отдельный Docker service без публичного порта
- owner authorization блокирует CRM context и OpenAI вызовы для неизвестных Telegram users
...
- Module `6`
- основные файлы: `main_telegram_ai.py`, `src/minimal_kanban/telegram_ai/*`, `tests/test_telegram_ai.py`
- focus: Telegram command quality, tool schema tightening, rollback/autopilot, voice/photo reliability
...
- AI quality зависит от качества OpenAI decision prompts and explicit CRM tool schemas
...
- Главный интеллектуальный модуль — `Module 6: Server AI Contour v1.7`
...
- При следующем AI-проходе наиболее ценный шаг — `Telegram live smoke + tool schema tightening`
