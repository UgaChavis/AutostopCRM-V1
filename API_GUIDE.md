...
Ответ использует схему `card_journal.v2`: `entries`/`timeline` дают машинно-читаемые события, `days`/`weeks`/`months` группируют их по времени, `totals` содержит счетчики, а `markdown`/`text` содержит человекочитаемый отчет в стиле кассового журнала. Для восстановимости изменений в `changes[]` и Markdown сохраняются полные блоки `до:` и `после:`.
