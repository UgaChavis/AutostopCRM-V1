...
3. `update_repair_order()` for header/client fields.
...
6. Disable with `set_card_ai_autofill(card_id, enabled=false)` when the follow-up window is no longer needed.
...
- VIN present and profile fields missing:
  Use `set_card_ai_autofill()` or manual agent task bound to the card.
- Clear part request:
