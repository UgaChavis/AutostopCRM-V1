...
DEFAULT_COLUMN_EMPTY_MESSAGES = {
    ...
    "done": "Готовых автомобилей пока нет.\nПеретащите сюда готовую машину.",
}
...
ONBOARDING_PAGES = [
    (
        ...
        "Это рабочая CRM автосервиса для карточек автомобилей, клиентов, заказ-нарядов, оплат и файлов. "
        "Локальное окно поднимает backend автоматически и хранит данные без ручного запуска серверов.",
    ),
    (
        "Как создать карточку",
        "Нажмите кнопку «Новая карточка». "
        "Укажите автомобиль, заголовок, подробности работ и срок возврата к задаче.",
    ),
    (
        "Как работать со сроками",
        "После сохранения карточки срок виден на доске. "
        "Индикатор помогает быстро найти просроченные и приближающиеся задачи.",
    ),
    (
        "Как читать индикатор",
        "Зелёный статус означает, что времени достаточно, жёлтый показывает приближение срока, "
        "а красный означает просрочку.",
    ),
    (
        "Как работать со столбцами",
        "На доске можно создавать новые столбцы кнопкой «Новый столбец». "
        "Карточки можно перетаскивать мышью между этапами работы.",
    ),
    (
        "Как редактировать и архивировать карточку",
        "Дважды щёлкните по карточке, чтобы открыть её. "
        "В окне карточки можно изменить содержание, срок и отправить закрытую задачу в архив.",
    ),
    (
        "Интеграции",
...
SETTINGS_CONNECTION_SUMMARY_TEMPLATE = "Локальный API: {local_api}\nMCP: {mcp}\nOpenAI: {openai}"
