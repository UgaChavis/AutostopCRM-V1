...
def build_chatgpt_connect_payload(
    ...
    *,
    ...
    lines = [
        ...
        "8c. For client work, call suggest_clients_for_card(card_id) or search_clients before create_client. Choose a concrete vehicles_preview[].id when known and pass it as client_vehicle_id to link_card_to_client; use upsert_client_vehicle for a new car of an existing client. Cards may stay unlinked when the customer is a one-off manual entry. delete_client is destructive and rejects linked cards unless allow_linked=true is explicitly confirmed.",
...
def build_chatgpt_connector_payload(settings: IntegrationSettings) -> str:
    payload = {
        ...
        "notes": [
            ...
            "For client work, use suggest_clients_for_card or search_clients before create_client; select a concrete client vehicle when possible; use upsert_client_vehicle for a new vehicle of an existing client; link_card_to_client should not overwrite manual client fields unless the operator confirms it; delete_client is destructive and should not be used on linked clients without explicit confirmation.",
