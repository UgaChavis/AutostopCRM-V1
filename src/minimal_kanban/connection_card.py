...
def build_chatgpt_connect_payload(
    ...
    *,
    ...
    lines = [
        ...
        "8c. For client work, call suggest_clients_for_card(card_id) or search_clients before create_client. Cards may stay unlinked when the customer is a one-off manual entry. delete_client is destructive and rejects linked cards unless allow_linked=true is explicitly confirmed.",
...
def build_chatgpt_connector_payload(settings: IntegrationSettings) -> str:
    payload = {
        ...
        "notes": [
            ...
            "For client work, use suggest_clients_for_card or search_clients before create_client; link_card_to_client should not overwrite manual card fields unless the operator confirms it; delete_client is destructive and should not be used on linked clients without explicit confirmation.",
