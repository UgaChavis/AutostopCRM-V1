...
def build_chatgpt_connector_payload(settings: IntegrationSettings) -> str:
    payload = {
        ...
        "notes": [
            ...
            "For client work, use suggest_clients_for_card or search_clients before create_client; select a concrete client vehicle when possible; use upsert_client_vehicle for a new or corrected vehicle of an existing client; use delete_client_vehicle to remove a car from the client profile without deleting cards; link_card_to_client should not overwrite manual client fields unless the operator confirms it; delete_client is destructive and should not be used on linked clients without explicit confirmation.",
