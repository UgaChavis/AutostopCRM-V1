    ...
    function quickAgentPrompts(context) {
      if (String(context?.kind || '').trim().toLowerCase() === 'card') {
        return [
          ...
          { label: 'ПОРЯДОК', prompt: 'Наведи порядок в этой карточке: структурируй описание без потери информации, автоматически заполни недостающие поля по возможности, предложи обновления для краткой сути, тегов и паспорта автомобиля, но не выдумывай данные.' },
