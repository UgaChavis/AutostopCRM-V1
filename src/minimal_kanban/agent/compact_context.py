...
def _compact_lines(
    value: Any, *, limit: int = _MAX_LINES_PER_FIELD, line_limit: int = 160
) -> list[str]:
    lines: list[str] = []
    seen: set[str] = set()
...
def _extract_vehicle_facts(
    card: dict[str, Any], repair_order: dict[str, Any], description: str
) -> dict[str, Any]:
    vehicle_profile = (
...
def build_ai_wall_digest_packet(context_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in events[:18]:
        ...
        if len(facts) < _MAX_WALL_FACTS and any(
            marker in combined.casefold() for marker in _IMPORTANT_WALL_MARKERS
        ):
            facts.append(combined)
        if len(important_notes) < _MAX_WALL_NOTES:
            detail_note = _clean_text(
                details.get("vehicle") or details.get("title") or details.get("task_id"), limit=120
            )
            if detail_note:
...
def build_ai_card_context_packet(
    ...
    has_candidate_facts = any(
        item["kind"] in {"vin", "vehicle", "phone", "mileage"} for item in facts
    )
    ...
    return {
        ...
        "normalization_candidate": bool((wall_digest or {}).get("normalization_candidate"))
        or _text_has_normalization_opportunity(description),
    }
...
def build_ai_repair_order_context_packet(context_payload: dict[str, Any]) -> dict[str, Any] | None:
    ...
    repair_order = card.get("repair_order") if isinstance(card.get("repair_order"), dict) else {}
    repair_order_text = (
        context_payload.get("repair_order_text")
        if isinstance(context_payload.get("repair_order_text"), dict)
        else {}
    )
    ...
    materials = (
        repair_order.get("materials") if isinstance(repair_order.get("materials"), list) else []
    )
    ...
    payment_status = _coalesce(
        repair_order.get("payment_status"), repair_order.get("payment_method")
    )
    return {
        ...
        "label": _coalesce(
            repair_order.get("number"), repair_order.get("status"), card.get("title")
        ),
...
def build_ai_attachment_intake_packet(context_payload: dict[str, Any]) -> dict[str, Any]:
    attachments = (
        ...
        if isinstance(context_payload.get("attachments"), list)
        else []
    )
    repair_order_packet = build_ai_repair_order_context_packet(context_payload)
    linked_repair_order_id = str(
        (repair_order_packet or {}).get("repair_order_id", "") or ""
    ).strip()
    ...
    for attachment in attachments:
        ...
        ai_ready = (
            ...
            or extension in _DOCUMENT_EXTENSIONS
        ...
        items.append(
            {
                ...
                "ai_ready": ai_ready,
                "scope": {
                    "card_id": str(attachment.get("card_id", "") or "").strip()
                    or str((context_payload.get("card") or {}).get("id", "") or "").strip(),
