  ...
  <script>
    ...
    const els = {
      ...
      displayDashboardMessageToolbar: document.getElementById('displayDashboardMessageToolbar'),
      displayDashboardFontSize: document.getElementById('displayDashboardFontSize'),
      displayDashboardEmojiButton: document.getElementById('displayDashboardEmojiButton'),
      displayDashboardEmojiPalette: document.getElementById('displayDashboardEmojiPalette'),
      displayDashboardMessageEditor: document.getElementById('displayDashboardMessageEditor'),
      displayDashboardImageInput: document.getElementById('displayDashboardImageInput'),
      ...
      adminUserEmployeesReadAccess: document.getElementById('adminUserEmployeesReadAccess'),
      ...
      archiveSearchMeta: document.getElementById('archiveSearchMeta'),
      ...
      employeesReportPanel: document.getElementById('employeesReportPanel'),
      employeesReadOnlyNotice: document.getElementById('employeesReadOnlyNotice'),
      ...
      employeeSalaryReconciliationPeriodMode: document.getElementById('employeeSalaryReconciliationPeriodMode'),
      employeeSalaryReconciliationDaysInput: document.getElementById('employeeSalaryReconciliationDaysInput'),
      employeeSalaryReconciliationDaysField: document.getElementById('employeeSalaryReconciliationDaysField'),
      employeeSalaryReconciliationDateFromInput: document.getElementById('employeeSalaryReconciliationDateFromInput'),
      ...
      cashboxTransferPreview: document.getElementById('cashboxTransferPreview'),
    ...
    function hydrateEmployeesUiRefs() {
      ...
      els.employeesReportPanel = document.getElementById('employeesReportPanel');
      els.employeesReadOnlyNotice = document.getElementById('employeesReadOnlyNotice');
      ...
      els.employeeSalaryReconciliationPeriodMode = document.getElementById('employeeSalaryReconciliationPeriodMode');
      els.employeeSalaryReconciliationDaysInput = document.getElementById('employeeSalaryReconciliationDaysInput');
      els.employeeSalaryReconciliationDaysField = document.getElementById('employeeSalaryReconciliationDaysField');
      els.employeeSalaryReconciliationDateFromInput = document.getElementById('employeeSalaryReconciliationDateFromInput');
    ...
    function setOperatorLoginGateOpen(isOpen) {
      ...
      document.body.classList.toggle('operator-login-gate-open', open);
      els.identityModal?.classList.toggle('operator-login-gate', open);
      const workspaceShell = document.querySelector('.shell');
      const stickyDock = document.querySelector('.sticky-dock');
      [workspaceShell, stickyDock].forEach((element) => {
      ...
      if (els.identityModal) {
        els.identityModal.setAttribute('role', 'dialog');
        els.identityModal.setAttribute('aria-modal', open ? 'true' : 'false');
      }
    }
    ...
    function closeOperatorLoginModal() {
      setOperatorLoginGateOpen(false);
    ...
    function operatorCanViewEmployees() {
      return operatorCanAccessEmployeesCashboxes()
        || operatorHasPermission(EMPLOYEES_READ_ACCESS_PERMISSION);
    }

    function operatorHasEmployeesReadOnlyAccess() {
      return operatorCanViewEmployees() && !operatorCanAccessEmployeesCashboxes();
    }
    ...
    function requireEmployeesViewAccess() {
      if (operatorCanViewEmployees()) return true;
      if (!state.operatorSessionToken || !state.actor) {
        requireOperatorSession();
      } else {
        setStatus('НЕТ ПРАВА НА РАЗДЕЛ «СОТРУДНИКИ».', true);
      }
      return false;
    }
    ...
    function syncEmployeesCashboxesAccessUi({ clearCachedData = false } = {}) {
      const canAccessEmployeesCashboxes = operatorCanAccessEmployeesCashboxes();
      const canViewEmployees = operatorCanViewEmployees();
      const canResetSalaryBalance = operatorCanResetSalaryBalance();
      const previousEmployeesCashboxesAccess = Boolean(state.employeesCashboxesAccess);
      const previousEmployeesReadAccess = Boolean(state.employeesReadAccess);
      const accessChanged = previousEmployeesCashboxesAccess !== canAccessEmployeesCashboxes
        || previousEmployeesReadAccess !== canViewEmployees;
      if (accessChanged || clearCachedData) state.employeesCashboxesAccessRevision += 1;
      state.employeesCashboxesAccess = canAccessEmployeesCashboxes;
      state.employeesReadAccess = canViewEmployees;
      ...
      if (!canAccessEmployeesCashboxes) {
        ...
        if (typeof closeModalAndChildren === 'function') {
          ...
          if (previousEmployeesCashboxesAccess) closeModalAndChildren('employees');
        }
        if (state.mobileView === 'cashboxes') state.mobileView = 'board';
      }
      if (!canViewEmployees) {
        if (typeof closeModalAndChildren === 'function') closeModalAndChildren('employees');
        if (state.mobileMorePanel === 'employees') state.mobileMorePanel = '';
      }
      if (clearCachedData || accessChanged) clearEmployeesCashboxesModuleState();

      const cashboxEntries = [els.cashboxesButton];
      const employeeEntries = [els.employeesButton];
      const mobileCashboxEntries = els.mobileAppShell?.querySelectorAll?.('[data-mobile-view="cashboxes"]') || [];
      const mobileEmployeeEntries = els.mobileAppShell?.querySelectorAll?.('[data-mobile-open="employees"]') || [];
      mobileCashboxEntries.forEach((element) => cashboxEntries.push(element));
      mobileEmployeeEntries.forEach((element) => employeeEntries.push(element));
      cashboxEntries.forEach((element) => syncEmployeesCashboxesEntry(element, canAccessEmployeesCashboxes));
      employeeEntries.forEach((element) => syncEmployeesCashboxesEntry(element, canViewEmployees));
      if (els.mobileEmployeesPanel && !canViewEmployees) els.mobileEmployeesPanel.hidden = true;
      if (typeof syncEmployeesReadOnlyWorkspaceUi === 'function') syncEmployeesReadOnlyWorkspaceUi();
    ...
    function renderOperatorUsers(data) {
      ...
      els.adminUsersList.innerHTML = users.length
        ? users.map((user) => {
            ...
            const canViewEmployees = canAccessEmployeesCashboxes
              || permissions.includes(EMPLOYEES_READ_ACCESS_PERMISSION);
            ...
            return '<div class="operator-user-row">' +
              ...
              '<div class="operator-user-row__stats">' +
                ...
                '<span class="operator-user-chip">СОТРУДНИКИ: ПРОСМОТР: ' + (canViewEmployees ? 'ДА' : 'НЕТ') + '</span>' +
    ...
    function editOperatorUserPermissions(username) {
      ...
      if (els.adminUserEmployeesReadAccess) {
        els.adminUserEmployeesReadAccess.checked = permissions.includes(EMPLOYEES_READ_ACCESS_PERMISSION);
      }
    ...
    function syncOperatorAdminSalaryResetPermission() {
      ...
      if (els.adminUserEmployeesReadAccess) {
        if (canAccessEmployeesCashboxes) els.adminUserEmployeesReadAccess.checked = false;
        els.adminUserEmployeesReadAccess.disabled = canAccessEmployeesCashboxes;
        els.adminUserEmployeesReadAccess.setAttribute(
          'aria-disabled',
          canAccessEmployeesCashboxes ? 'true' : 'false',
        );
      }
    }
    ...
    function clientRowAriaLabel(client) {
      ...
      const parts = [
        ...
        'тип ' + type,
        compactPhoneLine(client, 'телефон не указан'),
      ];
      if (stats.repair_orders_total !== undefined) parts.push('заказ-нарядов ' + stats.repair_orders_total);
      if (stats.vehicles_total !== undefined) parts.push('автомобилей ' + stats.vehicles_total);
      if (stats.last_visit) parts.push('последний визит ' + formatDate(stats.last_visit));
    ...
    function clientRowChipsHtml(client) {
      ...
      return chips.length
        ? '<div class="client-row__chips">' + chips.map((item) => '<span class="client-row__chip" title="' + escapeHtml(item) + '">' + escapeHtml(item) + '</span>').join('') + '</div>'
        : '';
    }
    ...
    function renderClientProfileEmptyState() {
      ...
      setClientProfileEmptyState(true);
      if (els.clientProfileTitle) els.clientProfileTitle.textContent = 'КЛИЕНТ НЕ ВЫБРАН';
      if (els.clientProfilePhone) els.clientProfilePhone.textContent = 'ВЫБЕРИТЕ КЛИЕНТА ИЛИ УТОЧНИТЕ ПОИСК';
      if (els.clientDebtValue) els.clientDebtValue.textContent = '0 ₽';
      fillClientForm({ client_type: 'person', phones: [] });
      renderClientVehiclesList([]);
      if (els.clientOrdersList) els.clientOrdersList.innerHTML = '<div class="empty">ВЫБЕРИТЕ КЛИЕНТА, ЧТОБЫ УВИДЕТЬ ЗАКАЗ-НАРЯДЫ.</div>';
      renderClientsList();
    }
    ...
    function resetClientForm() {
      ...
      state.clientsActiveProfile = null;
      state.clientVehicleEditor = null;
      setClientProfileEmptyState(false);
      const blank = {
        client_type: 'person',
        last_name: '',
        first_name: '',
      ...
      try {
        ...
        if (!existingUser || editingPermissions) {
          ...
          if (!canAccessEmployeesCashboxes && canReadEmployees) {
            payload.permissions.push(EMPLOYEES_READ_ACCESS_PERMISSION);
          }
    ...
    function adjustBoardBounds() {
      ...
      const detachedExtraColumn = els.board.querySelector('.column--virtual.is-detached');
      if (detachedExtraColumn instanceof HTMLElement) {
        maxRight = Math.max(
          maxRight,
          detachedExtraColumn.offsetLeft + detachedExtraColumn.offsetWidth + viewportSlackX,
        );
        maxBottom = Math.max(
          maxBottom,
          detachedExtraColumn.offsetTop + detachedExtraColumn.offsetHeight + viewportSlackY,
        );
      }
    ...
    function normalizePersonalBoardPreferences(value) {
      ...
      return {
        extra_column: {
          ...
          is_detached: rawExtraColumn.is_detached === true,
          position: {
            x: Math.max(0, Math.round(finiteNumber(rawExtraColumn.position?.x, defaults.extra_column.position.x))),
            y: Math.max(0, Math.round(finiteNumber(rawExtraColumn.position?.y, defaults.extra_column.position.y))),
          },
          filter: {
    ...
    async function toggleExtraBoardColumnDetached() {
      const nextPreferences = personalBoardPreferences();
      const nextDetached = !nextPreferences.extra_column.is_detached;
      if (nextDetached) {
        const section = els.board?.querySelector('[data-virtual-column="extra"]');
        const scale = normalizeBoardScale(state.boardScale);
        nextPreferences.extra_column.position = {
          x: Math.max(0, Math.round(finiteNumber(section?.offsetLeft, 0) / scale)),
          y: Math.max(0, Math.round(finiteNumber(section?.offsetTop, 0) / scale)),
        };
      }
      nextPreferences.extra_column.is_detached = nextDetached;
      await savePersonalBoardPreferences(nextPreferences, {
        statusMessage: nextDetached
          ? 'ДОПОЛНИТЕЛЬНАЯ КОЛОНКА ОТКРЕПЛЕНА.'
          : 'ДОПОЛНИТЕЛЬНАЯ КОЛОНКА ВОЗВРАЩЕНА В РЯД.',
      });
    }

    function beginExtraBoardColumnDrag(event) {
      if (state.mobileLite || !extraBoardColumnIsDetached()) return;
      if (event.button !== 0 || !(event.target instanceof HTMLElement)) return;
      const handle = event.target.closest('[data-extra-column-drag-handle]');
      if (!(handle instanceof HTMLElement)) return;
      const position = extraBoardColumnPreferences().position;
      state.extraBoardColumnDrag = {
        active: true,
        pointerId: event.pointerId,
        startClientX: event.clientX,
        startClientY: event.clientY,
        startX: finiteNonNegativeNumber(position.x),
        startY: finiteNonNegativeNumber(position.y),
        moved: false,
      };
      els.board?.querySelector('[data-virtual-column="extra"]')?.classList.add('is-dragging');
      event.preventDefault();
      event.stopPropagation();
    }

    function extraBoardColumnDragPosition(event, drag = state.extraBoardColumnDrag) {
      const scale = normalizeBoardScale(state.boardScale);
      return {
        x: Math.max(0, Math.round(drag.startX + ((event.clientX - drag.startClientX) / scale))),
        y: Math.max(0, Math.round(drag.startY + ((event.clientY - drag.startClientY) / scale))),
      };
    }

    function moveExtraBoardColumnDrag(event) {
      const drag = state.extraBoardColumnDrag;
      if (!drag.active || drag.pointerId !== event.pointerId) return;
      const dx = event.clientX - drag.startClientX;
      const dy = event.clientY - drag.startClientY;
      if (!drag.moved && Math.abs(dx) + Math.abs(dy) > 4) drag.moved = true;
      if (!drag.moved) return;
      const position = extraBoardColumnDragPosition(event, drag);
      const scale = normalizeBoardScale(state.boardScale);
      const section = els.board?.querySelector('[data-virtual-column="extra"]');
      if (section instanceof HTMLElement) {
        section.style.left = Math.round(position.x * scale) + 'px';
        section.style.top = Math.round(position.y * scale) + 'px';
      }
      adjustBoardBounds();
      event.preventDefault();
    }

    async function endExtraBoardColumnDrag(event) {
      const drag = state.extraBoardColumnDrag;
      if (!drag.active || drag.pointerId !== event.pointerId) return;
      state.extraBoardColumnDrag = {
        active: false,
      ...
      els.board?.querySelector('[data-virtual-column="extra"]')?.classList.remove('is-dragging');
      if (!drag.moved) return;
      const nextPreferences = personalBoardPreferences();
      nextPreferences.extra_column.position = extraBoardColumnDragPosition(event, drag);
      await savePersonalBoardPreferences(nextPreferences, {
        statusMessage: 'ПОЛОЖЕНИЕ ДОПОЛНИТЕЛЬНОЙ КОЛОНКИ СОХРАНЕНО.',
      });
    }
    ...
    function normalizeRepairOrderRow(row) {
      ...
      return {
        ...
        work_quantity_snapshot: String(source.work_quantity_snapshot ?? '').trim(),
        work_price_snapshot: String(source.work_price_snapshot ?? '').trim(),
        work_total_snapshot: String(source.work_total_snapshot ?? '').trim(),
    ...
    function applyRepairOrderCardUpdate(updatedCard, fallbackOrder = {}) {
      const previousCard = state.activeCard?.id === updatedCard?.id ? state.activeCard : null;
      const responseOrder = updatedCard?.repair_order || fallbackOrder;
      const nextCard = {
        ...(previousCard || {}),
        ...(updatedCard || {}),
        repair_order: responseOrder,
      };
      state.activeCard = nextCard;
      state.activeCardIsFull = previousCard ? state.activeCardIsFull : Boolean(nextCard?.id);
      if (state.mobileCard?.id === nextCard?.id) state.mobileCard = nextCard;
      applySavedCardLocalPatch(nextCard);
      syncCardArchiveAction(nextCard);
      const nextOrder = repairOrderCardDraft(nextCard, responseOrder);
      applyRepairOrderToForm(nextOrder);
      refreshRepairOrderEntry(nextCard);
      return nextOrder;
    }
    ...
    function readRepairOrderRowElement(row) {
      ...
      return normalizeRepairOrderRow({
        ...
        work_quantity_snapshot: row.dataset.repairOrderWorkQuantity || '',
        work_price_snapshot: row.dataset.repairOrderWorkPrice || '',
        work_total_snapshot: row.dataset.repairOrderWorkTotal || '',
    ...
    function cardJournalStripIcon(value) {
      return String(value ?? '').replace(/^[\s\uFE0F\u200D\u2600-\u27BF\uD800-\uDFFF]+/g, '').trim();
    }
    ...
    function archiveCardSearchText(card) {
      return normalizeClientSearchText([
        ...
        stripDescriptionFormatting(card?.description || ''),
      ].filter(Boolean).join(' '));
    }

    function filteredArchiveCards() {
      const cards = Array.isArray(state.archiveCards) ? state.archiveCards : [];
      const query = normalizeClientSearchText(state.archiveQuery || '');
      if (!query) return cards;
      return cards.filter((card) => archiveCardSearchText(card).includes(query));
    }

    function renderArchiveSearchMeta(total, visible) {
      if (!els.archiveSearchMeta) return;
      const query = String(state.archiveQuery || '').trim();
      els.archiveSearchMeta.textContent = query
        ? ('НАЙДЕНО: ' + visible + ' ИЗ ' + total + ' ПО ПОКАЗАННЫМ ' + ARCHIVE_PREVIEW_LIMIT)
        : ('ПОКАЗАНО: ' + visible + ' ИЗ ' + total + ' ПОСЛЕДНИХ АРХИВНЫХ КАРТОЧЕК');
    }
    ...
    function renderCompactArchiveRows(cards) {
      return cards.map((card) => {
        ...
        const restoreLabel = 'Вернуть карточку ' + heading;
        return '<div class="archive-row archive-row--compact"><div class="archive-row__main"><div class="archive-row__title" title="' + escapeHtml(heading) + '">' + escapeHtml(heading) + '</div><div class="archive-row__summary" title="' + escapeHtml(compactDescription || 'Описание не указано') + '">' + escapeHtml(summary || 'Описание не указано') + '</div></div><div class="archive-row__side"><div class="archive-row__meta">АРХИВ: ' + escapeHtml(formatDate(card.updated_at)) + '</div><button class="btn" data-restore-card="' + escapeHtml(card.id) + '" aria-label="Вернуть карточку ' + escapeHtml(heading) + '" title="' + escapeHtml(restoreLabel) + '">ВЕРНУТЬ</button></div></div>';
      }).join('');
    }
    ...
    function renderArchive() {
      const totalCards = Array.isArray(state.archiveCards) ? state.archiveCards : [];
      const cards = filteredArchiveCards();
      renderArchiveSearchMeta(totalCards.length, cards.length);
      els.archiveList.innerHTML = cards.length
        ...
        : String(state.archiveQuery || '').trim()
          ? '<div class="log-row__meta">ПО ПОКАЗАННЫМ АРХИВНЫМ КАРТОЧКАМ НИЧЕГО НЕ НАЙДЕНО.</div>'
    ...
    function renderExtraBoardColumnHtml(snapshot) {
      ...
      const settings = extraBoardColumnPreferences();
      const isDetached = settings.is_detached && !state.mobileLite;
      const scale = normalizeBoardScale(state.boardScale);
      const detachedClass = isDetached ? ' is-detached' : '';
      const detachedStyle = isDetached
        ? ' style="left:' + Math.round(settings.position.x * scale) + 'px;top:' + Math.round(settings.position.y * scale) + 'px;"'
        : '';
      const moveButton = isDetached
        ? '<button class="btn btn--ghost column--virtual__control column--virtual__move" type="button" data-extra-column-drag-handle="true" title="Перетащить колонку по рабочей области">ПЕРЕТАЩИТЬ</button>'
        : '';
    ...
    function renderCashboxesList() {
      ...
      els.cashboxesList.innerHTML = items.length ? items.map((item) => {
        ...
        const balanceMinor = cashboxBalanceMinor(item);
        const balanceDisplay = cashboxBalanceDisplay(item);
        const balanceSign = String(stats?.balance_sign || cashboxBalanceSign(balanceMinor));
        const activeClass = item.id === state.activeCashboxId ? ' is-active' : '';
