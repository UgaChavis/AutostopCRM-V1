...
class EmployeeFormulaUiTests(unittest.TestCase):
    def test_employee_cards_show_effective_formula(self) -> None:
        ...
        self.assertIn("% от стоимости заказ-наряда за наличный расчёт", BOARD_WEB_APP_HTML)
        self.assertIn("без безналичной надбавки, независимо от способа оплаты", BOARD_WEB_APP_HTML)
