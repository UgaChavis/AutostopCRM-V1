...
class JsonValuePolicyTests(unittest.TestCase):
    def test_bounded_lists_preserve_selection_coercion_and_shallow_copy(self) -> None:
        nested = {"child": 1}
    ...
    def test_mapping_lookup_preserves_order_depth_and_list_scan_limit(self) -> None:
    ...
    def test_api_storage_and_model_number_and_key_policies_remain_distinct(self) -> None:
    ...
    def test_depth_and_non_json_values_preserve_original_contract(self) -> None:
    ...
    def test_iterative_conversion_matches_previous_recursive_policy(self) -> None:
        def previous(value: Any, depth: int, nonfinite: float | None, drop_none_keys: bool) -> Any:
            if depth <= 0:
                return str(value)
    ...
    def test_full_state_depth_is_iterative_and_preserves_leaf_types(self) -> None:
        value = 17
        for _ in range(512):
            value = {"child": value}
