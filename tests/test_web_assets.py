...
class WebAssetsTests(unittest.TestCase):
    ...
    def test_web_assets_are_loaded_from_packaged_source_chunks(self) -> None:
        ...
        self.assertIn("minimal_kanban/web_app_assets/source", build_script)
        self.assertIn("minimal_kanban/static", build_script)
    ...
    def test_modal_ladder_stack_and_parent_close_cascade_are_wired(self) -> None:
        ...
        self.assertIn("'repair-order': () => closeRepairOrderModal()", close_fragment)
        self.assertIn(
            "'repair-order-payments': () => closeRepairOrderPaymentsModal()",
            close_fragment,
        )
        self.assertIn("if (result === false) return false;", close_fragment)
    ...
    def test_repair_order_print_module_exposes_preview_template_editor_and_routes(self) -> None:
        ...
        self.assertIn("['absent', 'active', 'reset_tombstone']", BOARD_WEB_APP_HTML)
        self.assertIn("Черновик сброшен · используются актуальные данные CRM", BOARD_WEB_APP_HTML)
        self.assertIn("draft.state === 'active' && draft.updated_at", BOARD_WEB_APP_HTML)
        self.assertIn("expected_source_fingerprint:", BOARD_WEB_APP_HTML)
