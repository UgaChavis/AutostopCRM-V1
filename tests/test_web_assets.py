...
class WebAssetsTests(unittest.TestCase):
    def test_board_brand_uses_autostop_name(self) -> None:
        ...
        self.assertIn('<div class="brand__title">AUTOSTOP / РџРЈР›Р¬Рў</div>', BOARD_WEB_APP_HTML)
        self.assertNotIn('<div class="brand__title">РљРђРќР‘РђРќ / РџРЈР›Р¬Рў</div>', BOARD_WEB_APP_HTML)
    ...
    def test_card_tag_editor_uses_compact_tag_controls(self) -> None:
        ...
        self.assertIn("РњР•РўРћРљ РќР•Рў", BOARD_WEB_APP_HTML)
        self.assertIn(" + ' Г—</button>'", BOARD_WEB_APP_HTML)
    ...
    def test_columns_expose_hidden_delete_button_with_guarded_flow(self) -> None:
        ...
        self.assertIn("РЈРґР°Р»РёС‚СЊ РїСѓСЃС‚РѕР№ СЃС‚РѕР»Р±РµС†", BOARD_WEB_APP_HTML)
        self.assertIn("window.confirm('РЈРґР°Р»РёС‚СЊ РїСѓСЃС‚РѕР№ СЃС‚РѕР»Р±РµС†", BOARD_WEB_APP_HTML)
    ...
    def test_archive_modal_uses_last_30_compact_rows(self) -> None:
        self.assertIn("РђР РҐРР’ / РџРћРЎР›Р•Р”РќРР• 30", BOARD_WEB_APP_HTML)
    ...
    def test_sticky_dock_uses_single_icon_button_without_dropdown(self) -> None:
        ...
        self.assertIn('aria-label="РќРѕРІС‹Р№ СЃС‚РёРєРµСЂ"', BOARD_WEB_APP_HTML)
        self.assertIn('title="РќРѕРІС‹Р№ СЃС‚РёРєРµСЂ"', BOARD_WEB_APP_HTML)
    ...
    def test_card_form_semantics_distinguish_make_model_and_short_essence(self) -> None:
        self.assertIn("const CARD_VEHICLE_FIELD_LABEL = 'РњР°СЂРєР° / РјРѕРґРµР»СЊ';", BOARD_WEB_APP_HTML)
        self.assertIn("const CARD_TITLE_FIELD_LABEL = 'РљСЂР°С‚РєР°СЏ СЃСѓС‚СЊ';", BOARD_WEB_APP_HTML)
        self.assertIn("const CARD_TITLE_REQUIRED_MESSAGE = 'РЈРљРђР–Р РљР РђРўРљРЈР® РЎРЈРўР¬ РљРђР РўРћР§РљР.';", BOARD_WEB_APP_HTML)
        self.assertIn("function configureCardFieldSemantics()", BOARD_WEB_APP_HTML)
        self.assertIn("vehicleLabel.textContent = 'РњРђР РљРђ / РњРћР”Р•Р›Р¬';", BOARD_WEB_APP_HTML)
        self.assertIn("els.cardVehicle.placeholder = 'Nissan Teana J32';", BOARD_WEB_APP_HTML)
        self.assertIn("titleLabel.textContent = 'РљР РђРўРљРђРЇ РЎРЈРўР¬';", BOARD_WEB_APP_HTML)
        self.assertIn("els.cardTitle.placeholder = 'РљСЂР°С‚РєР°СЏ СЃСѓС‚СЊ РїСЂРѕР±Р»РµРјС‹, Р·Р°РґР°С‡Рё РёР»Рё СЂРµР·СѓР»СЊС‚Р°С‚Р°';", BOARD_WEB_APP_HTML)
    ...
    def test_vehicle_panel_exposes_only_minimal_profile_fields(self) -> None:
        ...
        self.assertNotIn("Р”РѕРї. РґР°РЅРЅС‹Рµ:", BOARD_WEB_APP_HTML)
        self.assertNotIn("РђРќРђР›РР— РџРћР›Р•Р™ РљРђР РўРћР§РљР", BOARD_WEB_APP_HTML)
        ...
        self.assertNotIn("oem_notes', label: 'РљРѕСЂРѕС‚РєР°СЏ Р·Р°РјРµС‚РєР°'", BOARD_WEB_APP_HTML)
    ...
    def test_vehicle_panel_hides_empty_summary_and_first_group_title(self) -> None:
        ...
        self.assertIn("РџСЂРѕР±РµРі", BOARD_WEB_APP_HTML)
        self.assertIn("РўРµР»РµС„РѕРЅ РєР»РёРµРЅС‚Р°", BOARD_WEB_APP_HTML)
        self.assertIn("Р¤РРћ РєР»РёРµРЅС‚Р°", BOARD_WEB_APP_HTML)
        ...
        self.assertIn("if (profile.mileage) summaryLines.push('РџСЂРѕР±РµРі: ' + profile.mileage);", BOARD_WEB_APP_HTML)
    ...
    def test_card_timer_panel_uses_compact_number_fields(self) -> None:
        self.assertIn('<div class="panel-title">РћР‘Р РђРўРќР«Р™ РћРўРЎР§РЃРў</div>', BOARD_WEB_APP_HTML)
    ...
    def test_card_preview_uses_readable_russian_meta_labels(self) -> None:
        self.assertIn("Р‘Р•Р— РњР•РўРћРљ", BOARD_WEB_APP_HTML)
        self.assertIn("РћРїРёСЃР°РЅРёРµ РЅРµ СѓРєР°Р·Р°РЅРѕ", BOARD_WEB_APP_HTML)
        self.assertIn("РЎРР“Рќ", BOARD_WEB_APP_HTML)
        self.assertIn("Р¤РђР™Р›Р« ", BOARD_WEB_APP_HTML)
        self.assertIn("Р–РЈР РќРђР› ", BOARD_WEB_APP_HTML)
    ...
    def test_unread_cards_expose_corner_badge_and_hover_seen_flow(self) -> None:
        ...
        self.assertIn("title=\"РќРµ РїСЂРѕС‡РёС‚Р°РЅРѕ\"", BOARD_WEB_APP_HTML)
    ...
    def test_updated_cards_expose_yellow_badge_and_hover_seen_flow(self) -> None:
        ...
        self.assertIn("title=\"РћР±РЅРѕРІР»РµРЅРѕ\"", BOARD_WEB_APP_HTML)
    ...
    def test_operator_ui_exposes_login_profile_and_admin_routes(self) -> None:
        ...
        self.assertIn("РђРґРјРёРЅРёСЃС‚СЂР°С‚РѕСЂ СЃРѕР·РґР°РµС‚ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ РёР»Рё РѕР±РЅРѕРІР»СЏРµС‚ РµРјСѓ РїР°СЂРѕР»СЊ.", BOARD_WEB_APP_HTML)
        ...
        self.assertIn("РЎРўРђРўРРЎРўРРљРђ: 15 Р”РќР•Р™", BOARD_WEB_APP_HTML)
    ...
    def test_tag_editor_limits_cards_to_three_tags(self) -> None:
        ...
        self.assertIn("РќРђ РљРђР РўРћР§РљР• РњРћР–Р•Рў Р‘Р«РўР¬ РќР• Р‘РћР›Р•Р• 3 РњР•РўРћРљ.", BOARD_WEB_APP_HTML)
    ...
    def test_repair_order_modal_exposes_minimal_form_and_print_flow(self) -> None:
        ...
        self.assertNotIn('<div class="dialog__title-prefix">Р—РђРљРђР—-РќРђР РЇР”</div>', BOARD_WEB_APP_HTML)
        self.assertNotIn('<label for="repairOrderClient">РљР›РР•РќРў</label>', BOARD_WEB_APP_HTML)
        self.assertNotIn('<label for="repairOrderVehicle">РђР’РўРћРњРћР‘РР›Р¬</label>', BOARD_WEB_APP_HTML)
        self.assertNotIn('<label for="repairOrderComment">РРќР¤РћР РњРђР¦РРЇ Р”Р›РЇ РљР›РР•РќРўРђ</label>', BOARD_WEB_APP_HTML)
        ...
        self.assertIn('id="repairOrderPaymentsMethod"', BOARD_WEB_APP_HTML)
        ...
        self.assertIn('data-repair-order-total="prepayment"', BOARD_WEB_APP_HTML)
        ...
        self.assertIn("syncRepairOrderPaymentMethod(normalized.payment_method);", BOARD_WEB_APP_HTML)
        ...
        self.assertIn("payment_method: els.repairOrderPaymentsMethod?.value || els.repairOrderPaymentMethod?.value || 'cash',", BOARD_WEB_APP_HTML)
        ...
        self.assertIn('<section class="print-section"><h2>РРЅС„РѕСЂРјР°С†РёСЏ РґР»СЏ РєР»РёРµРЅС‚Р°</h2>', BOARD_WEB_APP_HTML)
        self.assertIn('РЎС‚РѕРёРјРѕСЃС‚СЊ Р·Р°РєР°Р·-РЅР°СЂСЏРґР°', BOARD_WEB_APP_HTML)
        self.assertIn('РќР°Р»РѕРіРё Рё СЃР±РѕСЂС‹', BOARD_WEB_APP_HTML)
        self.assertIn('РџСЂРµРґРѕРїР»Р°С‚Р°', BOARD_WEB_APP_HTML)
        self.assertIn('Рљ РґРѕРїР»Р°С‚Рµ', BOARD_WEB_APP_HTML)
    ...
    def test_repair_order_autofill_status_uses_report_hints(self) -> None:
        ...
        self.assertIn("С†РµРЅС‹ РёР· РёСЃС‚РѕСЂРёРё", BOARD_WEB_APP_HTML)
    ...
    def test_topbar_repair_orders_list_uses_compact_row_open_flow(self) -> None:
        ...
        self.assertIn("РЎРџРРЎРћРљ: Р”РђРўРђ / РђР’РўРћ / РЎРЈРўР¬ / РЎРЈРњРњРђ", BOARD_WEB_APP_HTML)
        self.assertIn("РћС‚РєСЂС‹С‚Р°", BOARD_WEB_APP_HTML)
        self.assertIn("РўРµР»РµС„РѕРЅ", BOARD_WEB_APP_HTML)
        self.assertIn("РђРІС‚РѕРјРѕР±РёР»СЊ", BOARD_WEB_APP_HTML)
        self.assertIn("РЎРјС‹СЃР» РєР°СЂС‚РѕС‡РєРё", BOARD_WEB_APP_HTML)
        self.assertIn("РЎСѓРјРјР°", BOARD_WEB_APP_HTML)
    ...
    def test_repair_orders_menu_supports_open_and_closed_filters(self) -> None:
        ...
        self.assertIn("РћРўРљР Р«РўР«Р•: ", BOARD_WEB_APP_HTML)
        self.assertIn("РђР РҐРР’: ", BOARD_WEB_APP_HTML)
        self.assertIn("repairOrdersIsClosedView(status)", BOARD_WEB_APP_HTML)
        self.assertIn("const phoneText = phone || '-';", BOARD_WEB_APP_HTML)
        self.assertIn("РЎС‚Р°С‚СѓСЃ", BOARD_WEB_APP_HTML)
        self.assertIn("РўРµР»РµС„РѕРЅ", BOARD_WEB_APP_HTML)
        self.assertIn("item.opened_at || item.created_at || item.date || item.updated_at", BOARD_WEB_APP_HTML)
        self.assertIn("item.grand_total, item.works_total", BOARD_WEB_APP_HTML)
