...
from minimal_kanban.api.server import ApiServer, RequestContextFactory


class RejectedBodyDrainTests(unittest.TestCase):
    @contextmanager
    def connection(self, length: int) -> Iterator[tuple[SimpleNamespace, socket.socket]]:
    ...
    def test_only_declared_body_is_consumed_and_timeout_is_restored(self) -> None:
    ...
    def test_declared_size_cannot_exceed_drain_cap(self) -> None:
    ...
    def test_slow_body_has_a_total_deadline(self) -> None:
        with self.connection(1000000) as (handler, client):
...
class RejectedBodyHttpTests(CardServiceCase):
    def test_delayed_cross_origin_body_returns_403_without_creating_card(self) -> None:
