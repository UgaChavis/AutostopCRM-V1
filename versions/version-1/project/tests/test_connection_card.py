...
class ConnectionCardTests(unittest.TestCase):
    ...
    def test_chatgpt_connect_payload_contains_key_values(self) -> None:
        ...
        self.assertIn("connector_display_name = Minimal Kanban / This Board Only (public.example)", text)
        self.assertIn("connector_scope_rule = current Minimal Kanban board only", text)
    ...
    def test_connector_and_responses_payloads_are_built_from_live_settings(self) -> None:
        ...
        self.assertEqual(derive_connector_display_name(settings), "Minimal Kanban / This Board Only (kanban.example)")
        self.assertEqual(connector_data["name"], "Minimal Kanban / This Board Only (kanban.example)")
