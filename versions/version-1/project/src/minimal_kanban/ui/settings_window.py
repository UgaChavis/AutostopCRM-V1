...
class ChatGPTConnectDialog(QDialog):
    ...
    def _refresh_from_settings(self, settings: IntegrationSettings) -> None:
        ...
        self.effective_mcp_url_input.setText(self._display_value(settings.mcp.effective_mcp_url))
    ...
    def _set_status(self, message: str, *, tone: str = "info") -> None:
        self.preflight_status_label.setText(message)
        self.preflight_status_label.setProperty("tone", tone)
        self.style().unpolish(self.preflight_status_label)
        self.style().polish(self.preflight_status_label)
...
class SettingsWindow(QDialog):
    ...
    def _apply_runtime_state_to_diagnostics(self, state, *, status_override: str | None = None) -> None:
        try:
            settings = self._settings_service.save(self._collect_settings())
        ...
        updated = self._settings_service.apply_test_result(settings, "mcp", result)
        saved = self._settings_service.save(updated)
        self._load_into_form(saved)
    ...
    def _copy_connection_card(self) -> None:
        settings = self._settings_service.normalize(self._collect_settings())
    ...
    def _export_connection_card(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Экспорт карточки подключения", str(default_path), "Text Files (*.txt)")
        if not path:
            return
        settings = self._settings_service.normalize(self._collect_settings())
        text = build_connection_card(
        ...
        self._set_status(f"Карточка подключения экспортирована: {path}", tone="success")
    ...
    def _export_chatgpt_connector_payload(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            self,
            "Экспорт ChatGPT connector JSON",
            str(default_path),
            "JSON Files (*.json)",
        )
        if not path:
        ...
        self._set_status(f"ChatGPT connector JSON экспортирован ({mode}): {path}", tone="success")
    ...
    def _export_responses_payload(self) -> None:
        ...
        Path(path).write_text(payload, encoding="utf-8")
        self._set_status(f"Responses API JSON экспортирован: {path}", tone="success")
    ...
    def _export_settings_snapshot(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Экспорт настроек интеграции", str(default_path), "JSON Files (*.json)")
        if not path:
            return
        settings = self._settings_service.normalize(self._collect_settings())
        payload = build_settings_export(settings, include_secrets=self.include_secrets_checkbox.isChecked())
        Path(path).write_text(payload, encoding="utf-8")
        self._set_status(f"Настройки интеграции экспортированы: {path}", tone="success")
