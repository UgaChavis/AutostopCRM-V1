...
from ..models import AuditEvent, Card, Column, DEFAULT_COLUMN_IDS, StickyNote
...
class JsonStore:
    ...
    def read_bundle(self) -> dict[str, Any]:
        with self._lock:
            with self._process_lock.acquire():
                ...
                if columns_repaired or cards_repaired or stickies_repaired or events_repaired or settings_repaired:
                    state = {
    ...
    def _normalize_cards(self, state: dict, columns: list[Column]) -> tuple[list[Card], bool]:
        ...
        for index, item in enumerate(raw_cards):
            if not isinstance(item, dict):
                ...
                continue
            card = Card.from_dict(item, valid_columns=valid_column_ids, default_column=default_column_id)
            cards.append(card)
    ...
    def _normalize_cards_payload(self, cards: list[Card], columns: list[Column]) -> list[Card]:
        ...
        return [Card.from_dict(card.to_storage_dict(), valid_columns=valid_columns, default_column=default_column) for card in cards]
    ...
    def _normalize_events_payload(self, events: list[AuditEvent]) -> list[AuditEvent]:
        return [AuditEvent.from_dict(event.to_dict()) for event in events]
