...
def build_chatgpt_connect_payload(
    ...
    *,
    ...
    lines = [
        ...
        "9. If the task is about a vehicle card, use autofill_vehicle_data before create_card or update_card to assemble a normalized vehicle_profile draft from text and/or an image payload.",
        ...
        "12. Do not ask GPT to edit cards until it confirms connector_name, resource_url, board_name, and scope_rule.",
        "13. For mass column migrations, use bulk_move_cards instead of many sequential move_card calls.",
    ]
    if connector_auth_mode == "oauth_embedded":
        lines.extend(
            [
                "14. For the ChatGPT connector you normally do not need to paste a bearer token manually: the server exposes embedded OAuth / DCR metadata.",
                "15. If ChatGPT asks for linking, complete the embedded OAuth flow.",
                "16. The legacy bearer token below is only for Responses API or a manual MCP client.",
            ]
        )
    else:
        lines.extend(
            [
                "14. Bearer token is not required because MCP auth is disabled.",
                "15. Press Connect and verify that the tool list is visible.",
            ]
        )
...
def build_chatgpt_connector_payload(settings: IntegrationSettings) -> str:
    payload = {
        ...
        "notes": [
            ...
            "For vehicle cards, call autofill_vehicle_data before create_card or update_card when you need to derive vehicle_profile from text or an image payload.",
