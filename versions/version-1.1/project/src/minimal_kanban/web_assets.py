...
""",
    ...
    .overview-layout { display: grid; grid-template-columns: minmax(0, 1.22fr) minmax(300px, 0.92fr); gap: 12px; align-items: start; }
    .overview-main { display: grid; gap: 12px; }
    .overview-main__meta { display: grid; grid-template-columns: minmax(230px, 0.78fr) minmax(0, 1fr); gap: 10px; align-items: start; }
    ...
    @media (max-width: 900px) {
      ...
      .dialog--card { transform: none; width: min(1120px, 100%); }
          ...
          <aside class="subpanel vehicle-panel">
            <div class="vehicle-panel__head">
              <div class="stack" style="gap:6px;">
                ...
                <div class="vehicle-panel__flags" id="vehiclePanelFlags"></div>
              </div>
              <button class="btn btn--ghost" id="vehicleAutofillButton">АВТОЗАПОЛНИТЬ</button>
            </div>
            <div class="vehicle-autofill">
              <div class="field field--compact">
                <label for="vehicleAutofillText">ТЕКСТ / ДАННЫЕ ПО АВТО</label>
                <textarea id="vehicleAutofillText" class="vehicle-autofill__textarea" maxlength="8000" placeholder="Suzuki Swift 2014, VIN, двигатель, коробка, жидкости, проблема..."></textarea>
              </div>
              <div class="vehicle-autofill__actions">
                <div class="field field--compact" style="min-width:min(260px,100%);">
                  <label for="vehicleAutofillImage">ФОТО / СКРИН С ДАННЫМИ</label>
                  <input id="vehicleAutofillImage" type="file" accept="image/*">
                </div>
                <div class="vehicle-autofill__hint">ТЕКСТ → OCR → WEB ENRICH → SAFE MERGE</div>
              </div>
              <div class="vehicle-autofill__status is-empty" id="vehicleAutofillStatus">Автозаполнение пока не запускалось.</div>
            </div>
    ...
    const VEHICLE_FIELD_GROUPS = [
      {
        title: 'Идентификация',
        fields: [
          { name: 'make_display', label: 'Марка', placeholder: 'Suzuki' },
          { name: 'model_display', label: 'Модель', placeholder: 'Swift' },
          { name: 'generation_or_platform', label: 'Поколение / платформа', placeholder: 'AZG / ZC72S', wide: true },
          { name: 'production_year', label: 'Год', type: 'number', min: '1900', max: '2100', step: '1', placeholder: '2014' },
          { name: 'vin', label: 'VIN', placeholder: 'JSA...', copy: true, mono: true, wide: true, maxlength: '17' },
        ],
      },
      {
        title: 'Двигатель',
        fields: [
          { name: 'engine_code', label: 'Код двигателя', placeholder: 'K12B', copy: true, mono: true },
          { name: 'engine_model', label: 'Модель двигателя', placeholder: 'Gamma 1.6 MPI', copy: true },
          { name: 'engine_displacement_l', label: 'Объём, л', type: 'number', step: '0.1', placeholder: '1.6' },
          { name: 'engine_power_hp', label: 'Мощность, л.с.', type: 'number', step: '1', placeholder: '123' },
          { name: 'fuel_type', label: 'Топливо', placeholder: 'gasoline' },
        ],
      },
      {
        title: 'Трансмиссия',
        fields: [
          { name: 'gearbox_type', label: 'Тип коробки', placeholder: 'automatic' },
          { name: 'gearbox_model', label: 'Модель коробки', placeholder: 'JF016E', copy: true, mono: true },
          { name: 'drivetrain', label: 'Привод', placeholder: 'FWD / AWD' },
        ],
      },
      {
        title: 'Жидкости',
        fields: [
          { name: 'oil_engine_capacity_l', label: 'Масло двигателя, л', type: 'number', step: '0.1', placeholder: '3.7' },
          { name: 'oil_gearbox_capacity_l', label: 'Масло коробки, л', type: 'number', step: '0.1', placeholder: '6.7' },
          { name: 'coolant_capacity_l', label: 'Охлаждение, л', type: 'number', step: '0.1', placeholder: '5.2' },
        ],
      },
      {
        title: 'Прочее',
        fields: [
          { name: 'steering_system_type', label: 'Рулевое', placeholder: 'electric power steering', wide: true },
          { name: 'brake_front_type', label: 'Передние тормоза', placeholder: 'ventilated disc' },
          { name: 'brake_rear_type', label: 'Задние тормоза', placeholder: 'drum / disc' },
          { name: 'wheel_bolt_pattern', label: 'Разболтовка', placeholder: '4x100', copy: true, mono: true },
          { name: 'oem_notes', label: 'OEM / примечания', kind: 'textarea', placeholder: 'Краткие заводские и справочные замечания.', wide: true },
        ],
      },
      {
        title: 'Источники / достоверность',
        fields: [
          { name: 'source_summary', label: 'Источник', kind: 'textarea', placeholder: 'VIN decode + структурированный каталог', wide: true },
          { name: 'source_confidence', label: 'Confidence', type: 'number', min: '0', max: '1', step: '0.01', placeholder: '0.82' },
          { name: 'data_completion_state', label: 'Состояние', kind: 'select', options: VEHICLE_COMPLETION_OPTIONS },
          { name: 'source_links_or_refs', label: 'Ссылки / refs', kind: 'textarea', placeholder: 'https://... или catalog:minimal_kanban/...', wide: true },
        ],
      },
    ];
...
""",
      ...
      vehiclePanelFlags: document.getElementById('vehiclePanelFlags'),
      vehicleProfileFields: document.getElementById('vehicleProfileFields'),
      vehicleAutofillText: document.getElementById('vehicleAutofillText'),
      vehicleAutofillImage: document.getElementById('vehicleAutofillImage'),
    ...
    function renderVehicleProfileFields() {
      els.vehicleProfileFields.innerHTML = VEHICLE_FIELD_GROUPS.map((group) => {
        ...
        return '<section class="vehicle-group">' +
          '<div class="vehicle-group__title">' + escapeHtml(group.title) + '</div>' +
    ...
    function refreshVehiclePanel() {
      ...
      const summaryLines = [];
      const display = vehicleDisplayFromProfile(profile);
      if (display) summaryLines.push(display);
      if (profile.vin) summaryLines.push('VIN: ' + profile.vin);
      if (!summaryLines.length) summaryLines.push('Техкарта ещё не заполнена.');
      els.vehiclePanelSummary.textContent = summaryLines.join('\n');

      const flags = [];
      if (profile.data_completion_state) flags.push({ text: vehicleCompletionLabel(profile.data_completion_state), tone: 'accent' });
      if (profile.source_confidence !== null && profile.source_confidence !== undefined && profile.source_confidence !== '') {
        flags.push({ text: 'confidence ' + Math.round(Number(profile.source_confidence) * 100) + '%', tone: 'default' });
      }
      if (profile.autofilled_fields?.length) flags.push({ text: 'авто ' + profile.autofilled_fields.length, tone: 'default' });
      if (profile.manual_fields?.length) flags.push({ text: 'ручных ' + profile.manual_fields.length, tone: 'default' });
      if (vinLooksSuspicious(profile.vin)) flags.push({ text: 'vin проверить', tone: 'warning' });
      els.vehiclePanelFlags.innerHTML = flags.map((flag) => '<span class="vehicle-flag' + (flag.tone === 'accent' ? ' vehicle-flag--accent' : '') + (flag.tone === 'warning' ? ' vehicle-flag--warning' : '') + '">' + escapeHtml(flag.text) + '</span>').join('');
    ...
    async function autofillVehicleProfile() {
      const rawText = String(els.vehicleAutofillText.value || '').trim();
      const imageFile = els.vehicleAutofillImage.files?.[0] || null;
      if (!rawText && !imageFile && !els.cardVehicle.value.trim() && !els.cardTitle.value.trim() && !els.cardDescription.value.trim()) {
        renderVehicleAutofillStatus('НУЖЕН ТЕКСТ, ФОТО ИЛИ УЖЕ ВВЕДЁННЫЕ ДАННЫЕ КАРТОЧКИ.', true);
        return;
      }
      try {
        els.vehicleAutofillButton.disabled = true;
        renderVehicleAutofillStatus('СОБИРАЮ И НОРМАЛИЗУЮ ДАННЫЕ АВТОМОБИЛЯ...', false);
        const payload = {
          raw_text: rawText,
        ...
        if (imageFile) {
          ...
          payload.image_base64 = arrayBufferToBase64(buffer);
    ...
    function openCardModal(card) {
      ...
      state.vehicleProfileBaseline = cloneVehicleProfile(card?.vehicle_profile || {});
      applyVehicleProfileToForm(card?.vehicle_profile || emptyVehicleProfile());
      els.vehicleAutofillText.value = '';
      els.vehicleAutofillImage.value = '';
      renderTags();
      renderFiles(card);
      renderLogs([]);
      setTab('overview');
