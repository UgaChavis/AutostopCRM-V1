...
from ..vehicle_profile import VEHICLE_PRIMARY_FIELDS, VehicleProfile, normalize_vehicle_field_names
...
class CardService:
    ...
    def update_card(self, payload: dict) -> dict:
        with self._lock:
            updated_fields = {"vehicle", "title", "description", "deadline", "tags", "vehicle_profile"} & set(payload.keys())
            if not updated_fields:
                self._fail(
                    "validation_error",
                    "Для обновления карточки нужно передать хотя бы одно поле: vehicle, title, description, deadline или tags.",
                    details={"fields": ["vehicle", "title", "description", "deadline", "tags", "vehicle_profile"]},
                )
            bundle = self._store.read_bundle()
            cards = bundle["cards"]
            events = bundle["events"]
    ...
    def _append_vehicle_profile_wall_lines(self, lines: list[str], card: dict, *, indent: str = "    ") -> None:
        ...
        if not isinstance(profile, dict):
            return
        if not any(profile.get(field_name) for field_name in VEHICLE_PRIMARY_FIELDS):
            return
        ...
        engine_line = " | ".join(
            part
            for part in (
                str(profile.get("engine_code") or "").strip(),
                str(profile.get("engine_model") or "").strip(),
                f"{profile.get('engine_displacement_l')}L" if profile.get("engine_displacement_l") else "",
                f"{profile.get('engine_power_hp')}hp" if profile.get("engine_power_hp") else "",
            )
            if part
        )
        if engine_line:
        ...
        transmission_line = " | ".join(
            part
            for part in (
                str(profile.get("gearbox_type") or "").strip(),
    ...
    def _merge_vehicle_profile_patch(self, existing: VehicleProfile, value) -> tuple[VehicleProfile, list[str]]:
        ...
        return self._vehicle_profiles.merge_profile_patch(
            existing,
            incoming,
