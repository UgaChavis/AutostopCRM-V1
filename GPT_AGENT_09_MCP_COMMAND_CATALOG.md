...
Autofill-support surfaces already covered by existing MCP tools:
- `get_card(card_id)` for the full current card payload.
- `get_card_context(card_id)` for recent events, repair-order text, and focused context.
- `search_cards(...)` for lightweight related-card board context.
- `agent_actions(...)` for the operational AI log tail.
- `agent_tasks(...)` and `agent_runs(...)` for task and run tracking.

No extra MCP write tool is required for the current orchestration. Card enrichment still writes through `update_card()` and autofill control stays in `set_card_ai_autofill()`.
...
- `get_card_context(card_id, event_limit=20, include_repair_order_text=true)`
  Focused card context: card, recent events, attachments, repair-order text, and board context.
- `get_board_snapshot(archive_limit?)`
  Full board snapshot with columns, cards, and stickies.
- `get_board_context()`
...
- Use scheduled-task tools for `once`, `interval`, and `on_create`; do not emulate schedules manually.
